...
$SPLUNK_HOME/etc/apps/network_profiler/lookups/homenet_hosts.csv
	...
	This file is used by a saved search (hostmap_gen) that generates a lookup table that lists
	MAC address, IP, and hostname.  By default, this search runs every 10 minutes so MAC,IP, and
	hostname pairings should always be pretty fresh.  If you can't wait 10 minutes for this 
	search to run on its own, you can run it manually from Settings->Searches, Reports, and Alerts->hostmap_gen
